...
export async function generateHint(params: { name_ja: string; traits: Trait[]; difficulty: Difficulty }): Promise<string> {
  ...
  const traitGuide4 = traits.some(t => t.trait_type === ('climate' as Trait['trait_type'])) ? '気候や地理的特徴を描写。' : ''
  const traitGuide5 = traits.some(t => t.trait_type === ('culture' as Trait['trait_type'])) ? '文化的背景や歴史を表現。' : ''
